...
function FinancePanel({ data, currency, selectedAccountId, selectedHoldingId, onCurrency, onSelectAccount, onSelectHolding, onBackAccount, onBackHolding, onNewTransaction, onEditTransaction, onDeleteTransaction, onNewAccount, onEditAccount, onNewHolding, onEditHolding, onNewRecurring, onEditRecurring, onDeleteRecurring, onRunRecurring, onToggleRecurring, onSettleReimbursement, onSettleAccount, onNewRate, onEditRate, onDeleteRate, onRefreshQuotes }: FinancePanelProps) {
  ...
  return <div className="page finance-page">
    ...
    <section className="section-block"><div className="section-title"><div><span>EXCHANGE RATES</span><h2>人民币汇率</h2></div><div className="section-actions"><button type="button" onClick={onRefreshQuotes}>每日更新</button><button type="button" onClick={onNewRate}>＋ 手动录入</button></div></div>{data.exchangeRates.length ? <div className="plain-list">{data.exchangeRates.map((rate) => <article key={rate.currency}><span>{rate.currency}</span><div><strong>1 {rate.currency} = ¥ {money(rate.cnyRate)}</strong><small>{rate.source === 'daily' ? '每日参考' : '手动确认'} · 适用 {rate.asOf} · 更新 {rate.updatedAt.replace('T', ' ')}</small></div><div className="row-ops"><button type="button" className="edit" onClick={() => onEditRate(rate.currency)}>编辑</button><button type="button" className="del" onClick={() => onDeleteRate(rate.currency)}>删除</button></div></article>)}</div> : <div className="list-empty">尚未录入外币汇率；CNY 以 1.000000 自动折算，其他币种暂不计入总额。</div>}<p className="automation-note">每天 18:00 拉取当日参考汇率和收盘价，只更新估值，不会改流水。</p></section>
