...
export default function Home() {
  ...
  return <main className={`phone-app ${data.theme === 'dark' ? 'dark' : ''}`}>
    ...
    {tab === 'life' && <div className="page feature-page"><section className="feature-heading"><span>LIFE</span><h2>健康、出行和记忆</h2><p>完整记录进入二级页面，首页只保留摘要。</p></section><div className="feature-grid"><button onClick={() => navigate('schedule')}><span>日</span><strong>日程</strong><small>今天的安排</small></button><button onClick={() => navigate('health')}><span>健</span><strong>健康</strong><small>身高体重心率</small></button><button onClick={() => navigate('travel')}><span>行</span><strong>出行</strong><small>火车与航班</small></button><button onClick={() => navigate('memory')}><span>忆</span><strong>记忆</strong><small>管家记住什么</small></button><button onClick={() => navigate('butler')}><span>管</span><strong>管家</strong><small>本机摘要问答</small></button></div></div>}
    {tab === 'health' && <HealthPanel records={data.healthRecords} onAdd={() => setSheet('health')} onImport={importHealth} onSelectExport={chooseGadgetbridgeExport} onExportDiagnostics={exportHealthDiagnostics} onSaveBody={saveBodyMetrics} />}
    ...
    <nav className="bottom-nav" aria-label="主导航">{navItems.map((item) => <button key={item.id} className={tab === item.id || (item.id === 'life' && ['health', 'travel', 'memory', 'data', 'butler', 'vault', 'schedule'].includes(tab)) ? 'active' : ''} onClick={() => navigate(item.id)}><span className="nav-icon">{item.icon}{item.id === 'capture' && inboxPendingCount > 0 && <b className="nav-badge">{inboxPendingCount > 9 ? '9+' : inboxPendingCount}</b>}</span>{item.label}</button>)}</nav>
...
function HealthPanel({ records, onAdd, onImport, onSelectExport, onExportDiagnostics, onSaveBody }: { records: HealthRecord[]; onAdd: () => void; onImport: () => void; onSelectExport: () => void; onExportDiagnostics: () => void; onSaveBody: (height: number, weight: number) => void }) {
