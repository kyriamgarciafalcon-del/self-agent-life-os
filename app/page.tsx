...
import { addDaysKey, buildScopedSummary, detectLegacyDemoData, isBackupPayload, localDateKey, otherAssetCurrencies, parseNaturalCapture, totalAssets, weekDates } from './product-logic';
...
function normalizeData(raw: Partial<AppData>): AppData {
  ...
  const rawAccounts = (raw.accounts ?? []).map((account) => ({ ...account, currency: account.currency ?? 'CNY' as Currency }));
...
export default function Home() {
  ...
  const totalBalance = useMemo(() => totalAssets(data.accounts, 'CNY'), [data.accounts]);
  ...
  function saveTransaction(input: ExpenseDraft & { transactionKind?: 'expense' | 'income'; accountAmount?: number }) {
    ...
    const transaction: Transaction = { id: uid('transaction'), kind: transactionKind, amount: Math.abs(input.amount), accountAmount: Math.abs(input.accountAmount ?? input.amount), currency: input.currency, merchant: input.merchant, category: transactionKind === 'income' ? '收入' : input.category, accountId: input.accountId, source: input.source, reimbursable: transactionKind === 'expense' && input.reimbursable, createdAt: localStamp() };
  ...
  return <main className={`phone-app ${data.theme === 'dark' ? 'dark' : ''}`}>
    ...
    {tab === 'home' && <div className="page home-page">
      ...
      <section className="summary-grid"><button onClick={() => navigate('schedule')}><span>下一项 · {nextSchedule?.time ?? '空闲'}</span><strong>{nextSchedule?.title ?? '今天没有更多日程'}</strong><small>{data.schedules.filter((item) => item.date === TODAY && item.done).length}/{data.schedules.filter((item) => item.date === TODAY).length} 已完成</small></button><button onClick={() => navigate('finance')}><span>今日支出</span><strong>¥ {money(todaySpend)}</strong><small>总资产 ¥ {money(totalBalance)}</small></button></section>
...
function FinancePanel({ data, currency, selectedAccountId, selectedHoldingId, onCurrency, onSelectAccount, onSelectHolding, onBackAccount, onBackHolding, onNewTransaction, onEditTransaction, onDeleteTransaction, onNewAccount, onEditAccount, onNewHolding, onEditHolding, onNewRecurring, onEditRecurring, onDeleteRecurring, onRunRecurring, onToggleRecurring }: FinancePanelProps) {
  ...
  const currencyAccounts = data.accounts.filter((account) => account.currency === currency);
  const assets = totalAssets(data.accounts, currency);
  const otherCurrencies = otherAssetCurrencies(data.accounts, currency);
  return <div className="page finance-page">
    <section className="finance-toolbar"><div><span>{Number(MONTH.slice(0, 4))}年{Number(MONTH.slice(5, 7))}月</span><h2>财务</h2></div><select aria-label="统计币种" value={currency} onChange={(event) => onCurrency(event.target.value as Currency)}>{currencies.map((item) => <option key={item}>{item}</option>)}</select></section>
    <section className="asset-summary"><span>TOTAL ASSETS · {currency}</span><h2>总资产</h2><strong>{assets < 0 ? '−' : ''}{mark} {money(Math.abs(assets))}</strong><p>{currencyAccounts.length ? `${currencyAccounts.length} 个${currency}账户余额相加。改余额、记账或删账户后立刻更新。` : '还没有账户，合计为 0。添加账户后这里会显示真实加总。'}{otherCurrencies.length ? ` 另有 ${otherCurrencies.join('、')} 账户，切换右上角币种查看。` : ''}</p>{!currencyAccounts.length && <button type="button" onClick={onNewAccount}>添加第一个账户</button>}</section>
    ...
    <section className="section-block account-section"><div className="section-title"><div><span>ACCOUNTS</span><h2>我的账户</h2></div><button onClick={onNewAccount}>＋ 添加账户</button></div>{currencyAccounts.length || data.accounts.length ? <div className="account-grid">{data.accounts.map((account) => <article className={account.tone} key={account.id} role="button" tabIndex={0} onClick={() => onSelectAccount(account.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelectAccount(account.id); }}><span>{account.type} · {account.currency}</span><button className="account-edit-button" onClick={(event) => { event.stopPropagation(); onEditAccount(account.id); }}>编辑</button><h3>{account.name}</h3><strong>{account.balance < 0 ? '−' : ''}{currencyMark(account.currency)} {money(Math.abs(account.balance))}</strong><small className="account-open-hint">查看账单 ›</small></article>)}</div> : <div className="list-empty">还没有账户，总资产为 0</div>}</section>
    ...
    <section className="section-block"><div className="section-title"><div><span>LEDGER · {currency}</span><h2>本月流水</h2></div></div><TransactionList items={monthItems} accounts={data.accounts} onEdit={onEditTransaction} onDelete={onDeleteTransaction} /></section>
  </div>;
}
