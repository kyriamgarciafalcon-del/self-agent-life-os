...
import { accountRole, addDaysKey, applyLedger, buildScopedSummary, defaultCashId, defaultReceivableId, detectLegacyDemoData, isBackupPayload, isDebtRole, localDateKey, normalizeAccountBalance, parseNaturalCapture, wealthTotals, weekDates } from './product-logic';
...
type Transaction = { id: string; kind: TransactionKind; amount: number; accountAmount: number; currency: Currency; merchant: string; category: string; accountId: string; targetAccountId?: string; source: string; reimbursable: boolean; reimburseAccountId?: string; reimbursed?: boolean; createdAt: string };
...
type AppData = { demoMode: boolean; schedules: ScheduleItem[]; accounts: Account[]; transactions: Transaction[]; recurringRules: RecurringRule[]; healthRecords: HealthRecord[]; travels: TravelItem[]; investments: InvestmentHolding[]; memories: MemoryItem[]; privacy: PrivacySettings; vaultItems: VaultItem[]; theme: 'light' | 'dark' };
...
const accountTypes = ['资金账户', '储蓄卡', '现金', '信用卡', '理财账户', '储值账户', '订阅账户', '待收回', '报销账户', '欠款', '物品资产'];
const currencies: Currency[] = ['CNY', 'USD', 'HKD', 'EUR', 'JPY'];
function roleLabel(type: string) {
  const role = accountRole(type);
  if (role === 'receivable') return type.includes('报销') ? '应收 · 报销' : '应收';
  if (role === 'payable') return '应付 · 债务';
  if (role === 'liability') return '负债 · 信贷';
...
function formatAssetSummary(accounts: { type: string; currency: string; balance: number }[]) {
  const lines = wealthTotals(accounts);
  return lines.length ? lines.map((line) => formatAssetAmount(line.currency, line.net)).join(' · ') : '¥ 0.00';
}
...
export default function Home() {
  ...
  const totalBalanceLabel = useMemo(() => formatAssetSummary(data.accounts), [data.accounts]);
  ...
  function addSchedule(event: FormEvent<HTMLFormElement>) {
    ...
    setData((current) => ({ ...current, schedules: editing ? current.schedules.map((row) => row.id === editing ? { ...item, done: row.done, color: row.color } : row) : [...current.schedules, item] }));
  ...
  function saveTransaction(input: ExpenseDraft & { transactionKind?: 'expense' | 'income'; accountAmount?: number }) {
    ...
    const reimburseAccountId = reimbursable ? defaultReceivableId(data.accounts, input.currency) : undefined;
    if (reimbursable && !reimburseAccountId) { notify('请先添加报销账户或待收回账户'); return; }
    const transaction: Transaction = { id: uid('transaction'), kind: transactionKind, amount: Math.abs(input.amount), accountAmount: Math.abs(input.accountAmount ?? input.amount), currency: input.currency, merchant: input.merchant, category: transactionKind === 'income' ? '收入' : input.category, accountId: input.accountId, source: input.source, reimbursable, reimburseAccountId, reimbursed: false, createdAt: localStamp() };
  ...
  function addTransaction(event: FormEvent<HTMLFormElement>) {
    ...
    const reimburseAccountId = reimbursable ? (previous?.reimburseAccountId ?? defaultReceivableId(data.accounts, String(form.get('currency')) as Currency)) : undefined;
    if (reimbursable && !reimburseAccountId) { notify('请先添加报销账户或待收回账户'); return; }
    const transaction: Transaction = { id: previous?.id ?? uid('transaction'), kind, amount: Math.abs(amount), accountAmount: Math.abs(amount * Number(form.get('exchangeRate') || 1)), currency: String(form.get('currency')) as Currency, merchant: String(form.get('merchant')), category: kind === 'income' ? '收入' : kind === 'transfer' ? '账户转账' : String(form.get('category')), accountId: String(form.get('accountId')), targetAccountId: kind === 'transfer' ? String(form.get('targetAccountId')) : undefined, source: previous?.source ?? '手动记录', reimbursable, reimburseAccountId, reimbursed: reimbursable ? previous?.reimbursed ?? false : false, createdAt: previous?.createdAt ?? localStamp() };
  ...
  function settleReimbursement(id: string) {
    ...
    const cashId = previous.accountId;
    const receivableId = previous.reimburseAccountId ?? defaultReceivableId(data.accounts, previous.currency);
    if (!receivableId) { notify('找不到报销账户'); return; }
    const transfer: Transaction = { id: uid('transaction'), kind: 'transfer', amount: previous.amount, accountAmount: previous.accountAmount, currency: previous.currency, merchant: `报销入账 · ${previous.merchant}`, category: '报销入账', accountId: receivableId, targetAccountId: cashId, source: '报销确认', reimbursable: false, reimbursed: false, createdAt: localStamp() };
    setData((current) => ({
      ...current,
      accounts: adjustAccounts(current.accounts, transfer, 1),
      transactions: [transfer, ...current.transactions.map((item) => item.id === previous.id ? { ...item, reimbursed: true } : item)],
    }));
    notify('报销已入账，资金账户已收回');
  }
  function settleAccount(id: string) {
    ...
    const cashId = defaultCashId(data.accounts, account.currency);
    if (!cashId || cashId === account.id) { notify('请先添加一个资金账户'); return; }
    if (role === 'receivable') {
      const transfer: Transaction = { id: uid('transaction'), kind: 'transfer', amount: account.balance, accountAmount: account.balance, currency: account.currency, merchant: `收回 ${account.name}`, category: /报销/.test(account.type) ? '报销入账' : '收回应收', accountId: account.id, targetAccountId: cashId, source: '应收结算', reimbursable: false, reimbursed: false, createdAt: localStamp() };
      setData((current) => ({ ...current, accounts: adjustAccounts(current.accounts, transfer, 1), transactions: [transfer, ...current.transactions] }));
      notify(/报销/.test(account.type) ? '报销已全部收回' : '应收已全部收回');
      return;
    }
    if (isDebtRole(role)) {
      const cash = data.accounts.find((item) => item.id === cashId);
      const amount = Math.min(account.balance, Math.max(0, cash?.balance ?? 0));
      if (!amount) { notify('资金账户余额不足，无法还清'); return; }
      const transfer: Transaction = { id: uid('transaction'), kind: 'transfer', amount, accountAmount: amount, currency: account.currency, merchant: `偿还 ${account.name}`, category: '还款', accountId: cashId, targetAccountId: account.id, source: '债务还款', reimbursable: false, reimbursed: false, createdAt: localStamp() };
      setData((current) => ({ ...current, accounts: adjustAccounts(current.accounts, transfer, 1), transactions: [transfer, ...current.transactions] }));
      notify(amount < account.balance ? '已按资金余额部分还款' : '债务已还清');
    }
  }
  function addRecurringRule(event: FormEvent<HTMLFormElement>) {
    ...
    if (!data.accounts.some((account) => account.id === accountId)) { notify('请先添加并选择一个账户'); return; }
    if (data.accounts.find((account) => account.id === accountId)?.currency !== currency || (targetAccountId && data.accounts.find((account) => account.id === targetAccountId)?.currency !== currency)) { notify('账单币种需与相关账户币种一致'); return; }
  ...
  function addHealthRecord(event: FormEvent<HTMLFormElement>) {
    ...
    const record: HealthRecord = { id: uid('health'), kind, value: Number(form.get('value')), note: String(form.get('note') || (kind === 'sleep' ? '睡眠' : kind === 'exercise' ? '运动' : '饮食')), createdAt: localStamp() };
  ...
  return <main className={`phone-app ${data.theme === 'dark' ? 'dark' : ''}`}>
    ...
    {tab === 'home' && <div className="page home-page">
      ...
      <section className="section-block"><div className="section-title"><div><span>FEATURES</span><h2>生活工具</h2></div></div><div className="feature-grid"><button onClick={() => navigate('travel')}><span>行</span><strong>出行</strong><small>火车与航班</small></button><button onClick={() => navigate('health')}><span>健</span><strong>健康</strong><small>睡眠与运动</small></button><button onClick={() => navigate('butler')}><span>管</span><strong>管家</strong><small>本机摘要问答</small></button><button onClick={() => navigate('data')}><span>数</span><strong>数据</strong><small>统一趋势</small></button><button onClick={() => navigate('vault')}><span>钥</span><strong>密码库</strong><small>安全元数据</small></button></div></section>
    ...
    {tab === 'vault' && <VaultPanel items={nativeOn && vaultMeta.length ? vaultMeta : data.vaultItems} nativeOn={nativeOn} />}
...
function DataPanel({ data }: { data: AppData }) {
  ...
  const hasEnough = month.length + todayItems.length + sleepValues.length > 0;
  return <div className="page feature-page"><section className="data-hero"><span>本机摘要</span><h2>{hasEnough ? '已保存记录' : '暂无结论'}</h2><p>{hasEnough ? '只汇总你确认保存过的日程、账本和健康记录，没有预测分数。' : '记录几天真实数据后，这里才会出现摘要。'}</p></section><section className="feature-section"><div className="feature-title"><div><span>SUMMARY</span><h2>统一摘要</h2></div></div><div className="data-summary"><article><span>健</span><div><strong>健康</strong><small>{sleepValues.length ? `平均睡眠 ${avgSleep.toFixed(1)} 小时 · ${sleepValues.length} 条` : '还没有睡眠记录'}</small></div><b>{sleepValues.length ? `${avgSleep.toFixed(1)}h` : '—'}</b></article><article><span>财</span><div><strong>财务</strong><small>{month.length ? `收入 ¥${money(income)} · 支出 ¥${money(expense)}` : '本月还没有流水'}</small></div><b>{month.length ? `${month.length} 笔` : '—'}</b></article><article><span>行</span><div><strong>行动</strong><small>今日 {todayItems.length} 项日程</small></div><b>{todayItems.length ? `${todayItems.filter((item) => item.done).length}/${todayItems.length}` : '—'}</b></article></div></section></div>;
}
...
function ButlerPanel({ data, ai }: { data: AppData; ai: AiConfig }) {
  ...
  return <div className="page butler-page"><section className="butler-intro"><span>LOCAL BUTLER</span><h2>{ai.baseUrl ? '已接入你的 AI 接口。' : '先从本机摘要开始。'}</h2><p>密码不会进入模型。健康与财务建议仅供参考。</p></section><div className="chat-suggestions"><button onClick={() => send('我现在最该做什么？')}>我现在最该做什么？</button><button onClick={() => send('分析本月财务')}>分析本月财务</button><button onClick={() => send('最近睡眠怎么样？')}>最近睡眠怎么样？</button></div><div className="chat-messages">{messages.map((message, index) => <div key={index} className={message.role}>{message.text}</div>)}</div><form className="butler-composer" onSubmit={(event) => { event.preventDefault(); send(); }}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder={busy ? '正在生成…' : '问问今天的状态…'} /><button disabled={busy}>发送</button></form></div>;
}
...
function FinancePanel({ data, currency, selectedAccountId, selectedHoldingId, onCurrency, onSelectAccount, onSelectHolding, onBackAccount, onBackHolding, onNewTransaction, onEditTransaction, onDeleteTransaction, onNewAccount, onEditAccount, onNewHolding, onEditHolding, onNewRecurring, onEditRecurring, onDeleteRecurring, onRunRecurring, onToggleRecurring, onSettleReimbursement, onSettleAccount }: FinancePanelProps) {
  ...
  if (selectedAccount) {
    ...
    return <div className="page finance-page account-detail-page"><button className="inline-back" onClick={onBackAccount}>‹ 返回全部账户</button><section className={`account-hero ${selectedAccount.tone}`}><span>{roleLabel(selectedAccount.type)} · {selectedAccount.currency}</span><h2>{selectedAccount.name}</h2><strong>{isDebtRole(accountRole(selectedAccount.type)) ? '欠 ' : ''}{currencyMark(selectedAccount.currency)} {money(normalizeAccountBalance(selectedAccount.type, selectedAccount.balance))}</strong><button onClick={() => onEditAccount(selectedAccount.id)}>编辑账户与余额</button>{accountRole(selectedAccount.type) === 'receivable' && selectedAccount.balance > 0 && <button onClick={() => onSettleAccount(selectedAccount.id)}>{/报销/.test(selectedAccount.type) ? '全部报销入账' : '全部收回应收'}</button>}{isDebtRole(accountRole(selectedAccount.type)) && selectedAccount.balance > 0 && <button onClick={() => onSettleAccount(selectedAccount.id)}>用资金账户还款</button>}</section><section className="account-stats"><article><span>流入</span><strong>+{currencyMark(selectedAccount.currency)}{money(income)}</strong></article><article><span>流出</span><strong>−{currencyMark(selectedAccount.currency)}{money(expense)}</strong></article><article><span>账目</span><strong>{accountItems.length} 笔</strong></article></section>{selectedAccount.type === '理财账户' && <section className="section-block"><div className="section-title"><div><span>HOLDINGS</span><h2>该账户持仓</h2></div><button onClick={onNewHolding}>＋ 添加产品</button></div><InvestmentList items={accountHoldings} onSelect={onSelectHolding} /></section>}<section className="section-block"><div className="section-title"><div><span>ACCOUNT LEDGER</span><h2>该账户全部账单</h2></div><button onClick={onNewTransaction}>＋ 记一笔</button></div><TransactionList items={accountItems} accounts={data.accounts} onEdit={onEditTransaction} onDelete={onDeleteTransaction} onSettle={onSettleReimbursement} /></section></div>;
  }
  ...
  const wealth = wealthTotals(data.accounts);
  return <div className="page finance-page">
    ...
    <section className="asset-summary"><span>NET WORTH</span><h2>净资产</h2><div className="asset-lines">{wealth.length ? wealth.map((line) => <strong key={line.currency}>{formatAssetAmount(line.currency, line.net)}</strong>) : <strong>¥ 0.00</strong>}</div>{wealth.map((line) => <p key={`${line.currency}-break`}>{line.currency} 资产 {formatAssetAmount(line.currency, line.assets)} · 应收 {formatAssetAmount(line.currency, line.receivable)} · 负债 {formatAssetAmount(line.currency, line.liability + line.payable)}</p>)}<p>{data.accounts.length ? '净资产 = 资金资产 + 应收 − 信贷负债 − 应付欠款。报销进应收，欠款是债务，不会被当成现金。' : '还没有账户，净资产为 0。'}</p>{!data.accounts.length && <button type="button" onClick={onNewAccount}>添加第一个账户</button>}</section>
    ...
    <section className="section-block investment-section"><div className="section-title"><div><span>INVESTMENTS · {currency}</span><h2>理财与投资</h2></div><button onClick={onNewHolding}>＋ 添加产品</button></div><div className="investment-toolbar"><span>价格由你手动保存，账户余额按持仓计算</span></div><InvestmentList items={data.investments.filter((item) => item.currency === currency)} onSelect={onSelectHolding} /><p className="automation-note">当前未连接行情服务，不会声称实时更新。点进产品后可手动更新价格和查看保存日期。</p></section>
