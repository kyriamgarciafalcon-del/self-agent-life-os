...
import { addDaysKey, assetTotals, buildScopedSummary, detectLegacyDemoData, isBackupPayload, localDateKey, parseNaturalCapture, weekDates } from './product-logic';
...
function formatAssetSummary(accounts: { currency: string; balance: number }[]) {
  const lines = assetTotals(accounts);
  return lines.length ? lines.map((line) => formatAssetAmount(line.currency, line.amount)).join(' · ') : '¥ 0.00';
}
...
export default function Home() {
  ...
  return <main className={`phone-app ${data.theme === 'dark' ? 'dark' : ''}`}>
    ...
    {tab === 'home' && <div className="page home-page">
      ...
      <section className="summary-grid"><button onClick={() => navigate('schedule')}><span>下一项 · {nextSchedule?.time ?? '空闲'}</span><strong>{nextSchedule?.title ?? '今天没有更多日程'}</strong><small>{data.schedules.filter((item) => item.date === TODAY && item.done).length}/{data.schedules.filter((item) => item.date === TODAY).length} 已完成</small></button><button onClick={() => navigate('finance')}><span>今日支出</span><strong>¥ {money(todaySpend)}</strong><small>总资产 {totalBalanceLabel}</small></button></section>
...
function FinancePanel({ data, currency, selectedAccountId, selectedHoldingId, onCurrency, onSelectAccount, onSelectHolding, onBackAccount, onBackHolding, onNewTransaction, onEditTransaction, onDeleteTransaction, onNewAccount, onEditAccount, onNewHolding, onEditHolding, onNewRecurring, onEditRecurring, onDeleteRecurring, onRunRecurring, onToggleRecurring }: FinancePanelProps) {
  ...
  const assetLines = assetTotals(data.accounts);
  return <div className="page finance-page">
    ...
    <section className="asset-summary"><span>TOTAL ASSETS</span><h2>总资产</h2><div className="asset-lines">{assetLines.length ? assetLines.map((line) => <strong key={line.currency}>{formatAssetAmount(line.currency, line.amount)}</strong>) : <strong>¥ 0.00</strong>}</div><p>{data.accounts.length ? `${data.accounts.length} 个账户全部计入，不随右上角币种切换。多币种会原币并列，不会把美元加进人民币。改余额、记账或删账户后立刻更新。` : '还没有账户，合计为 0。添加账户后这里会显示全部账户加总。'}</p>{!data.accounts.length && <button type="button" onClick={onNewAccount}>添加第一个账户</button>}</section>
