...
describe('finance UI structure', () => {
  ...
  it('restructures the finance page into accessible tabs with one add entry and a settlement sheet', () => {
    ...
    expect(shipped).not.toContain("const monthExpense = monthItems.filter((item) => item.kind === 'expense')");
    expect(page).toContain('inboxConfirmBlockReason');
    expect(finance).toContain('data.investments');
    expect(finance).toContain('现金');
    expect(finance).toContain('市值');
    expect(finance).toContain('账户现金由流水产生，持仓市值单独估值');
    expect(shipped).not.toContain('账户余额按持仓计算');
    expect(page).not.toContain("'订阅账户'");
    expect(page).not.toMatch(/if \(dest && accountRole\(dest\.type\) === 'receivable'\) return undefined/);
    expect(shipped).not.toContain('syncInvestmentBalances');
