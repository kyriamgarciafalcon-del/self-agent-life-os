...
function FinancePanel({ data, currency, selectedAccountId, selectedHoldingId, onCurrency, onSelectAccount, onSelectHolding, onBackAccount, onBackHolding, onNewTransaction, onEditTransaction, onDeleteTransaction, onNewAccount, onEditAccount, onNewHolding, onEditHolding, onNewRecurring, onEditRecurring, onDeleteRecurring, onRunRecurring, onSettleReimbursement, onSettleAccount, onNewRate, onEditRate, onDeleteRate, onRefreshQuotes }: FinancePanelProps) {
  ...
  const chartDays = Array.from({ length: Math.min(6, currentDay) }, (_, index) => Math.max(1, currentDay - 5) + index).map((day) => ({ day, amount: getMonthlyReport(transactionsInPeriod(monthItems, `${MONTH}-${String(day).padStart(2, '0')}`), currency).expense }));
  ...
  return <div className="page finance-page">
    ...
    <section className="section-block account-section" role="tabpanel" hidden={financeSection !== '账户'}><div className="section-title"><div><span>ACCOUNTS</span><h2>我的账户</h2></div><button onClick={onNewAccount}>＋ 添加账户</button></div>{data.accounts.length ? <div className="account-grid">{data.accounts.map((account) => <article className={account.tone} key={account.id}><span>{roleLabel(account.type)} · {account.currency}</span><button type="button" className="account-edit-button" aria-label={`编辑 ${account.name}`} onClick={() => onEditAccount(account.id)}>编辑</button><h3>{account.name}</h3><strong>{isDebtRole(accountRole(account.type)) ? '欠 ' : ''}{currencyMark(account.currency)} {money(normalizeAccountBalance(account.type, account.balance))}</strong><button type="button" className="account-open-button" aria-label={`查看 ${account.name} 的账单`} onClick={() => onSelectAccount(account.id)}>查看账单 ›</button></article>)}</div> : <div className="list-empty">还没有账户，净资产为 0</div>}</section>
